...
const RestaurantDating = (props) => {
  ...
  const onPressNextButton = () => {
   if(selectedTime && coin.length) {
    dispatch(
        setRegisterFormData({
          keyName: REGISTER_STATE_KEY_NAMES.restaurantDate,
          value: {
            date_type: DATING_TYPE.RESTAURANT,
            time: selectedTime.value,
            coins: coin,
          },
        }),
      );
    navigate(SCREENS_NAME.LUNCH_DATING);
   }
   else{
    ...
    <LayoutWrapper>
      <ScreenContainer
        ...
        <View style={styles.container}>
          <Dating 
           headingLabel={"Restaurant"}
           subHeading={"Let's stay together a while."}
           image={require('../assets/restaurantDating.png')}
...
export default RestaurantDating;