...
import {
  REGISTER_STATE_KEY_NAMES,
  setRegisterFormData,
} from '../redux/slices/registerSlices';
import { goBack, navigate } from '../navigation/NavigationService';

const CoffeeDating = props => {
  ...
  const onPressNextButton = () => {
    if (selectedTime && coin.length) {
      dispatch(
        setRegisterFormData({
          keyName: REGISTER_STATE_KEY_NAMES.coffeeDate,
          value: {
            date_type: DATING_TYPE.COFFEE_DATE,
            time: selectedTime.value,
            coins: coin,
          },
        }),
      );
      navigate(SCREENS_NAME.MOVIE_DATING);
    } else {
    ...
    <LayoutWrapper>
      <ScreenContainer
        ...
        <View style={styles.container}>
          <Dating
            headingLabel={'Coffee Date'}
            subHeading={'Coffee dates and sweet memories.'}
            image={require('../assets/coffeeDating.png')}
